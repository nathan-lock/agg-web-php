            ...
            if($attempt) {
                ...
                <script>
                  $(document).ready(function(){
                      Swal.fire({
                      title: 'Success!',
                      text: "User created, please login and await you're welcome email!",
                      icon: 'success',
                      confirmButtonText: 'Thanks!'
                    });
                  });
                </script>
                <!-- Sign up Send Grid API -->
                <?php
                $email = new \SendGrid\Mail\Mail();
                $email->setFrom("nathan.lock@coderus.com", "Active Gym Group");
                $email->setSubject("Registration");
                $email->addTo($_POST['email'], "Example User");
                $email->addContent("text/plain", "and easy to do anywhere, even with PHP");
                $email->addContent(
                    "text/html", "Welcome to Active Gym Group!"
                );
                $sendgrid = new \SendGrid('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');
                try {
...
?>
<div class="container">
    <h1 class="mb-4 pb-2">Register</h1>
